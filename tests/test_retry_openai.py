...
def make_openai_client(sequence):
    """Create a stub OpenAI client.

    sequence: list of Exception or strings (mapped to response.output_text)
    """
    calls = {"n": 0}
...
def test_statement_api_error_then_success(monkeypatch):
    client = make_openai_client([RuntimeError("boom"), "true"])
...
def test_statement_non_boolean_then_success(monkeypatch):
    client = make_openai_client(["maybe", "false"])
    wrapper = OpenAiWrapper(
        client=client,
        model="test-model",
...
def test_statement_exhaust_retries_raises(monkeypatch):
    client = make_openai_client(["maybe", "maybe", "maybe"])
    wrapper = OpenAiWrapper(
        client=client,
        model="test-model",
...
def test_function_type_mismatch_then_success(monkeypatch):
    ...
    client = make_openai_client(["not an int", "8"])
    wrapper = OpenAiWrapper(
        client=client,
        model="test-model",
